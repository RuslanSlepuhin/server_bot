...
class HHGetInformationAPI:
    ...
    async def get_info(self, how_much_pages=6, separator="+") -> bool:
        ...
        for self.page_number in range(1, how_much_pages-1):
    ...
    async def get_content_from_link(self, return_raw_dictionary=False):
        ...
        for link in self.list_links:
            vacancy_url = link.get('url') # alternate_url if checking hh.ru
    ...
    async def get_vacancy_data(self, vacancy_url, return_raw_dictionary):
        vacancy_response = requests.get(vacancy_url, headers=self.user_agent)
