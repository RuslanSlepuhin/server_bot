...
class RabotaGetInformation:
    ...
    async def get_info(self):
        ...
        for word in self.search_words:
            ...
            if self.bot_dict:
                await self.bot.send_message(
                    self.chat_id, link, disable_web_page_preview=True
                )
            ...
            for self.page_number in range(1, till):
                try:
                    await self.bot.send_message(
                        self.chat_id,
                        f"https://rabota.by/search/vacancy?text={self.word}&from=suggest_post&salary=&area=16&no_magic=true&ored_clusters=true&enable_snippets=true&search_period=1&page={self.page_number}&hhtmFrom=vacancy_search_list",
        ...
        if self.bot_dict:
            await self.bot.send_message(
                self.chat_id, "rabota.by parsing: Done!", disable_web_page_preview=True
            )

    async def get_link_message(self, raw_content):
        soup = BeautifulSoup(raw_content, "lxml")

        self.list_links = soup.find_all("a", class_="bloko-link")
                ...
                if "rabota.by" in vacancy_url:
                    ...
                    if found_vacancy:
                        ...
                        try:
                            ...
                            try:
                                body = soup.find(
                                    "div", class_="vacancy-description"
                                ).get_text()
                            except:
                                return False
                        if body:
                        ...
                        english = ""
                        if re.findall(r"[Аа]нглийский", tags) or re.findall(
                            r"[Ee]nglish", tags
                        ):
                            english = "English"
                        ...
                        try:
                            salary = soup.find(
                                "div", attrs={"data-qa": "vacancy-salary"}
                            ).get_text()
                        except:
                            salary = ""
                        ...
                        try:
                            experience = (
                                soup.find("p", class_="vacancy-description-list-item")
                                .find("span")
                                .get_text()
                            )
                        except:
                        ...
                        raw_content_2 = soup.findAll(
                            "p", class_="vacancy-description-list-item"
                        )
                        counter = 1
                        job_type = ""
                        ...
                        try:
                            city = (
                                soup.find("p", {"data-qa": "vacancy-view-location"})
                                .get_text()
                                .split(",")[0]
                            )
                        except:
                        ...
                        if english and (
                            "upper" in english_additional
                            or "b1" in english_additional
                            or "b2" in english_additional
                            or "internediate" in english_additional
                            or "pre" in english_additional
                        ):
