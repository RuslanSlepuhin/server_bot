...
class RemotehubGetInformation:
    ...
    async def get_info(self):
        ...
        if self.bot_dict:
            await self.bot.send_message(
                self.chat_id,
                f"https://www.remotehub.com/jobs/search?sort_type=2",
    ...
    async def get_link_message(self, raw_content):
        ...
        self.list_links = soup.find_all(
            "smp-landings-entity", class_="ng-star-inserted"
        )
        if self.list_links:
            if self.bot_dict:
                ...
                if found_vacancy:
                    ...
                    if vacancy:
                        ...
                        if body:
                            tags = ""
                            try:
                                raw_tags = soup.find_all(
                                    "span", class_="truncate muted-2"
                                )
                                tags = ""
                                for tag in raw_tags:
                                    tags += tag.text
                            ...
                            try:
                                company = soup.find(
                                    "div", class_="account-name mat-body-2"
                                ).text
                            except:
                                company = ""
                            ...
                            try:
                                salary = soup.find(
                                    "div", class_="price ng-star-inserted"
                                )
                                if not salary:
                                    salary = soup.find(
                                        "span", class_="label muted ng-star-inserted"
                                    )
                                salary = salary.text
                            ...
                            try:
                                date = soup.find(
                                    "div", class_="mat-body-2 posted-ago muted-2"
                                ).text.split()[0]
                            except:
                                date = ""
                            if date:
