...
class HHKzGetInformation:
    ...
    def __init__(self, bot_dict, search_word=None, **kwargs):
    ...
    async def get_info(self):
        ...
        for word in self.search_words:
            ...
            for self.page_number in range(1, till):
                try:
                    await self.bot.send_message(self.chat_id, f'https://hh.kz/search/vacancy?text={word}&from=suggest_post&salary=&schedule=remote&no_magic=true&ored_clusters=true&enable_snippets=true&search_period=1&excluded_text=&page={self.page_number}&hhtmFrom=vacancy_search_list',
                                          disable_web_page_preview=True)
                    self.browser.get(f'https://hh.kz/search/vacancy?text={word}&from=suggest_post&salary=&schedule=remote&no_magic=true&ored_clusters=true&enable_snippets=true&search_period=1&excluded_text=&page={self.page_number}&hhtmFrom=vacancy_search_list')
                    self.browser.execute_script("window.scrollTo(0, document.body.scrollHeight);")
