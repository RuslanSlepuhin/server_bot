...
params = {
    "english_level": (
        "[Ee]nglish[\W]{0,2}[A-Za-zА-Яа-я][\W]{0,3}[\d]",
        "[Uu]pper[\W]{0,1}intermediate",
        "[Ii]ntermediate",
        "[Pp]re[\W]{0,1}[Ii]ntermediate",
        "[Uu]pper",
    ),
}
...
class RemocateGetInformation:
    ...
    async def get_info(self):
        ...
        self.current_session = (
            await self.helper_parser_site.get_name_session() if self.db else None
        )

        if self.bot_dict:
            await self.bot.send_message(
                self.chat_id, "https://www.remocate.app/", disable_web_page_preview=True
            )
        ...
        while True:
            ...
            full_height = self.browser.execute_script(
    ...
    async def get_content_from_link(self):
        ...
        for link in self.list_links:
            ...
            category_element = link.find_element(
                By.CSS_SELECTOR, "div.job-card-bubble[fs-cmsfilter-field='category']"
            )
            ...
            if check_vacancy_not_exists:
                ...
                if found_vacancy:
                    ...
                    # get body --------------------------
                    try:
                        body_elements = self.browser.find_elements(
                            By.CSS_SELECTOR, "div.text-rich-text.w-richtext"
                        )
                        body = " ".join(
                            element.text.strip() for element in body_elements
                        )
                    except AttributeError as ex:
                        body = None
                        print(f"Exception occurred: {ex}")
                    ...
                    # get company -------------------------------
                    company = self.browser.find_element(
                        By.CLASS_NAME, "job-top-company"
                    ).text.strip()

                    # get job_type and relocation -------------------------------
                    job_info = self.browser.find_element(
                        By.CLASS_NAME, "job-top-tags"
                    ).text.strip()
                    ...
                    date = self.browser.find_element(
                        By.CLASS_NAME, "job-top-right"
                    ).text.strip()
                    date = convert_date_to_timestamp(date)

                    # get english -------------------------------
                    english = ""
                    if re.findall(r"[Аа]нглийский", body, re.IGNORECASE) or re.findall(
                        r"[Ee]nglish", body
                    ):
                        english_additional = ""
                        ...
                        if re.search(
                            r"\b(intermediate|upper|b1|b2|pre)\b",
                            english_additional,
                            re.IGNORECASE,
                        ):
                    ...
                    if not english and (
                        re.findall(r"[Аа]нглийский", body, re.IGNORECASE)
                        or re.findall(r"[Ee]nglish", body)
                    ):
                        english = "English"

                    # get city -------------------------------
                    city_info = (
                        self.browser.find_element(By.CLASS_NAME, "job-top-tags")
                        .text.strip()
                        .split()
                    )
                    city = ""
