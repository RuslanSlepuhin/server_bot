...
class HorecaBotMethods:
    ...
    async def send_short_cards(self, **kwargs) -> None:
        ...
        chat_id = kwargs['chat_id'] if 'chat_id' in kwargs else message.chat.id
    ...
    async def custom_send_edit_message(self, **kwargs) -> None:
        ...
        if kwargs['order_id'] not in self.main_class.message_dict:
            ...
            text = kwargs['text']
            self.main_class.message_dict[kwargs['order_id']] = await self.main_class.bot.send_message(chat_id=chat_id, text=text, reply_markup=markup)
        else:
            try:
