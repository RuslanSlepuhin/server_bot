...
async def product_project_manager_specialization_callback(
    query: CallbackQuery, state: FSMContext
) -> None:
    """Обработка callback для product_project_manager направления"""
    ...
    if query.data in buttons_product_project_manager:
        await confirm_choice_handler(
            query=query,
            selected_specializations=selected_specializations,
            buttons_dict=buttons_product_project_manager,
        )
